...
func numSubseq(nums []int, target int) int {
	...
	for left < len(nums)-1 && right < len(nums) {
		...
		// pp.Println(left)
		// pp.Println(right)
		// pp.Println(maxPQ[len(maxPQ)-1])
		// pp.Println(minPQ[len(minPQ)-1])

		for maxPQ[len(maxPQ)-1].appeared < left {
			// pp.Println("pop:", maxPQ[len(maxPQ)-1])
			_ = maxPQ.Pop()
		}
		for minPQ[len(minPQ)-1].appeared < left {
			_ = minPQ.Pop()
		}

		// pp.Println(len(maxPQ))
		// pp.Println("=== DEBUG END ========================================")

		max, min := maxPQ[len(maxPQ)-1].priority, minPQ[len(minPQ)-1].priority
