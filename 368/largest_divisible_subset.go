...
// June LeetCoding Challenge Day 13
...
func main() {
	...
	// fmt.Println([]int{1, 3, 9, 18, 54, 108, 540})
	fmt.Println([]int{1, 3, 9, 18, 90, 180, 360, 720})
	pp.Println("=========================================")
}
...
func largestDivisibleSubset(nums []int) []int {
	...
	for _, num := range nums {
		var divisible bool
		for i := range subsets {
			if num%subsets[i][len(subsets[i])-1] == 0 {
	...
	for i := range subsets {
		if len(subsets[i]) > len(subsets[largestIndex]) {
			largestIndex = i
		}
	}
	return subsets[largestIndex]
}
